...
import { type Channel, getChannel, updateChannel } from "@/lib/channels-storage"
...
export default function EditChannelPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (channelData) {
      ...
      setSelectedType(channelData.type)
      setSelectedPlatforms(channelData.platforms)

      setFormData({
        name: channelData.name,
        description: channelData.description || "",
        costPerImpression: channelData.costPerImpression ? channelData.costPerImpression.replace("€", "") : "",
        averageROI: channelData.averageROI ? channelData.averageROI.replace("%", "") : "",
  ...
  const handleTypeChange = (value: "Digital" | "Traditional" | "Other") => {
    ...
    // Don't reset platforms when editing, just change the type
  }
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    try {
      ...
      const updatedChannelData: Partial<Channel> = {
        name: formData.name,
        type: selectedType,
      ...
      // Update the channel in localStorage
      updateChannel(channel.id, updatedChannelData)
      ...
      alert("There was an error updating your channel. Please try again.")
    }
  }
          ...
          <div className="mb-6 flex items-center">
            <Button variant="ghost" size="sm" asChild className="mr-4">
              <Link href={`/channels/${channel.id}`}>
                <ChevronLeft className="mr-1 h-4 w-4" /> Back to Channel
              </Link>
            </Button>
            <h1 className="text-3xl font-bold tracking-tight">Edit Channel: {channel.name}</h1>
          </div>

          <form onSubmit={handleSubmit}>
            <Card>
              <CardHeader>
                <CardTitle>Edit Channel Details</CardTitle>
                <CardDescription>Update the information about your advertising channel</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value: "Active" | "Inactive" | "Archived") => handleStatusChange(value)}
                    >
                      <SelectTrigger>
                ...
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="outline" asChild>
                    <Link href={`/channels/${channel.id}`}>Cancel</Link>
                  </Button>
